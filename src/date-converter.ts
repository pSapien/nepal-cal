...
export function getMonthIndexByAliasName(monthAliases: Record<string, number>, input: string) {
  const normalized = input.trim().toLowerCase();
  if (normalized in monthAliases) {
    return monthAliases[normalized] + 1;
  }
...
export function getMonthName(monthNames: string[], monthIndex: number) {
